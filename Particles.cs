...
namespace StorybrewScripts
{
    public class Particles : StoryboardObjectGenerator
    {
        ...
        [Configurable] public Vector2 MaxScale = new Vector2(1, 1);
        [Configurable] public bool RandomScale = false;
        ...
        public override void Generate()
        {
            ...
            for (var i = 0; i < ParticleCount; i++)
            {
                ...
                if(RandomScale)
                {
                    if(MaxScale.X == MaxScale.Y)
                    {
                        var randomMaxScale = (float)Random(scale.X,MaxScale.X);
                        scale = new Vector2(randomMaxScale, randomMaxScale);
                    } else {
                        scale = new Vector2(
                            (float)Random(scale.X,MaxScale.X),
                            (float)Random(scale.Y,MaxScale.Y)
                        );
                    }
                }
